...
testcase1 = "familyof Dog { routine init(name,bread,age,color) {name := 2; breed := 1;age := 0; color := 3;}; routine dog(Name, Breed, Age, Color){name := Name; breed := Breed; age := Age; color := Color;}; OU GETTERS UO routine getName(){return name;};routine getBreed(){return breed;};routine getColor(){return color;}; routine getAge() {return age;}; OU SETTERS UO routine setName(Name){name := Name;};routine setBreed(Breed){breed := Breed;};routine setColor(Color){color := Color;};routine setAge(Age){age := Age;}; };dog1 := new Dog();panic(dog1.name);panic(dog1.breed);dog1.setName(4);panic(dog1.name);"
code2 = "square.length := square.getwidth()"

whenloop = "x:=10; when( x >= 0)  do{ panic(\"helloworld\");  x:=x-1; };"
...
testcae4 = "listen(x); panic(x);"
testcase5 = "FAMILYOF X{routine init(){name := 2;}};"
testcase7 = "routine add(x,y){ return x+y ;}; z:=add(2,3); panic(z);"
tesCase6 = """
    familyof Dog {
